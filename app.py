...
st.set_page_config(page_title="DataLens AI — Insights for Everyone", page_icon="📈", layout="wide")
...
def quick_forecast(df, date_col, value_col, periods=12):
    try:
        ...
        rows, last_date = [], d[date_col].iloc[-1]
...
st.caption("Upload a dataset, type a short prompt or chat in plain English, and auto-generate visuals. Ideal for students, small businesses, and startups.")
...
with st.sidebar:
    ...
    if st.button("Load startup_sales.csv (sample)"):
        try:
            file = io.BytesIO(SAMPLE_PATH.read_bytes()); file.name = "startup_sales.csv"
    ...
    st.header("About"); st.write("Runs locally or in the cloud. Your uploaded data stays within the app session.")

# Need a file
if not file:
    st.info("Upload a CSV/XLSX or click the sample button in the sidebar."); st.stop()
...
with st.expander("Column types & basic stats"):
    ...
    st.write("**Missing values (per column)**:"); st.write(df.isna().sum())

# Filters
df_work = df.copy()
if cats:
    choose_cats = st.multiselect("Filter by categorical columns (optional)", cats, default=[])
    for c in choose_cats:
        vals = st.multiselect(f"Keep values in '{c}'", sorted(df_work[c].dropna().unique().tolist()))
        if vals: df_work = df_work[df_work[c].isin(vals)]
if dts:
    date_col = dts[0]
...
try:
    if numerics:
        ...
        col1.metric("Rows", len(df_work)); col2.metric("Columns", df_work.shape[1]); col3.metric("Missing cells", int(df_work.isna().sum().sum()))
...
user_messages = []
if mode == "Prompt mode":
    prompt = st.text_input("Describe what you want to see (e.g., 'bar average age by gender' or 'pie percentage by Investment_Avenues')")
    if prompt: user_messages = [prompt]
else:
    st.subheader("Ask DataLens AI 💬")
    for m in st.session_state.chat_history:
        (st.chat_message("user") if m["role"]=="user" else st.chat_message("assistant")).write(m["text"])
